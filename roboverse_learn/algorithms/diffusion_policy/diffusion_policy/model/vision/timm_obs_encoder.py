...
class TimmObsEncoder(ModuleAttrMixin):
    def __init__(self,
        ...
        #         r=8,                          # 低秩矩阵的秩
        #         lora_alpha=16,                # 缩放系数（建议 alpha = 2*r）
        #         target_modules=["qkv", "fc1", "fc2"],  # 关键修正点！
        #         lora_dropout=0.1,             # 防止过拟合
        #         bias="none",                  # 不训练偏置
        #         modules_to_save=["head"]      # 分类头正常微调（非LoRA）
        #     )
        ...
        for key, attr in obs_shape_meta.items():
            ...
            if type == 'rgb':
                ...
                human_dim_keys.append(key) # not used in this class
            else:
                raise RuntimeError(f"Unsupported obs type: {type}")
        
        feature_map_shape = [x // downsample_ratio for x in image_shape]
        #  input = int (1 dim), output = float (32 dim)
        
        rgb_keys = sorted(rgb_keys)
        low_dim_keys = sorted(low_dim_keys)
        human_dim_keys = sorted(human_dim_keys)
        print('rgb keys:         ', rgb_keys)
        print('low_dim_keys keys:', low_dim_keys)
        print('human_dim_keys:   ', human_dim_keys)
        ...
        self.human_dim_keys = human_dim_keys
    ...
    def forward(self, obs_dict):
        ...
        for key in self.rgb_keys:
            img = obs_dict[key] # shape = (B, T, 3, 224, 224)
            B, T = img.shape[:2]
            assert B == batch_size
            ...
            # Sequential( 随机裁剪至212*212，然后resize至224*224，然后颜色增强
            ...
            # )
            # print(self.key_model_map[key])
            
            raw_feature = self.key_model_map[key](img) # [2*batch, 197, 768]
            feature = self.aggregate_feature(raw_feature) # [2*batch, 768]
            assert len(feature.shape) == 2 and feature.shape[0] == B * T 
            features.append(feature.reshape(B, -1)) # [batch, 1536]
        ...
        return result # [batch, 1568 + 32] (1568 = 1536 + 32 + 32)
    ...
    def output_shape(self):
        ...
        return example_output.shape # (1568 + X)
