...
class LowdimMaskGenerator(ModuleAttrMixin): # 反向传播时，不会更新mask，因为他是torch.no_grad()的
    def __init__(self,
        action_dim, obs_dim,
        # obs mask setup
        max_n_obs_steps=2, 
    ...
    @torch.no_grad()
    def forward(self, shape, seed=None): # 当shape确定时，mask也就确定了
        device = self.device
        B, T, D = shape
        assert D == (self.action_dim + self.obs_dim)

        # create all tensors on this device
        rng = torch.Generator(device=device) # random number generator
        if seed is not None:
            rng = rng.manual_seed(seed) # seed是一个整数，用于初始化生成器

        # generate dim mask
        dim_mask = torch.zeros(size=shape, 
            dtype=torch.bool, device=device) # dim_mask是一个bool型的tensor，shape是(B,T,D)，全是False
        is_action_dim = dim_mask.clone()
        is_action_dim[...,:self.action_dim] = True # is_action_dim的前action_dim列是True
        is_obs_dim = ~is_action_dim 

        # generate obs mask
        if self.fix_obs_steps: # fix_obs_steps是指是否固定观测步数
            obs_steps = torch.full((B,), 
            fill_value=self.max_n_obs_steps, device=device) # 生成一个全是max_n_obs_steps的tensor
        else:
            obs_steps = torch.randint(
                low=1, high=self.max_n_obs_steps+1, 
                size=(B,), generator=rng, device=device) # 生成一个随机的tensor，取值范围是[1, max_n_obs_steps+1)
            
        steps = torch.arange(0, T, device=device).reshape(1,T).expand(B,T)
        obs_mask = (steps.T < obs_steps).T.reshape(B,T,1).expand(B,T,D) # obs_mask是一个(B,T,D)的tensor，前obs_steps列是True，其余是False
        obs_mask = obs_mask & is_obs_dim # obs_mask只在观测维度上是True
        ...
        return mask # 最终的mask是一个(B,T,D)的tensor，只有观测维度是True，action_mask在avisible为True时才包含action维度
