...
from diffusion_policy.common.rnn_sampler import RNNSequenceSampler, get_val_mask
...
class UmiRNNDataset(BaseDataset):
    def __init__(self,
        shape_meta: dict,
        dataset_path: str,
        ...
        for key, attr in obs_shape_meta.items():
            ...
            elif attr_type == 'human_dim':
                humandim_keys.append(key)
        ...
        sampler = RNNSequenceSampler(
            shape_meta=shape_meta,
            replay_buffer=replay_buffer,
            humandim_keys=humandim_keys,
            rgb_keys=rgb_keys,
    ...
    def get_validation_dataset(self):
        ...
        val_set.sampler = RNNSequenceSampler(
            shape_meta=self.shape_meta,
            replay_buffer=self.replay_buffer,
            rgb_keys=self.rgb_keys,
            humandim_keys=self.humandim_keys,
            lowdim_keys=self.sampler_lowdim_keys,
            obs_horizon=self.obs_horizon,
            action_horizon=self.action_horizon,
            key_latency_steps=self.key_latency_steps,
            down_sample_steps=self.down_sample_steps,
    ...
    def get_normalizer(self, **kwargs) -> LinearNormalizer:
        ...
        for i in range(self.num_robot):
            action_normalizers.append(get_range_normalizer_from_stat(
                array_to_stats(data_cache['action'][..., i * dim_a: i * dim_a + 3])))             # pos
            action_normalizers.append(get_identity_normalizer_from_stat(
                array_to_stats(data_cache['action'][..., i * dim_a + 3: (i + 1) * dim_a - 1])))   # rot
            action_normalizers.append(get_range_normalizer_from_stat(
                array_to_stats(data_cache['action'][..., (i + 1) * dim_a - 1: (i + 1) * dim_a]))) # gripper
