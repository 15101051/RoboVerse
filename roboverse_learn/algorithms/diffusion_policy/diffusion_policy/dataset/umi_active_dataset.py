...
from diffusion_policy.common.sampler import SequenceSampler, get_val_state_mask
...
class UmiActiveDataset(BaseDataset):
    def __init__(self,
        shape_meta: dict,
        dataset_path: str,
        ...
        val_mask, state_mask = get_val_state_mask(
    ...
    def get_state_dataset(self):
        state_set = copy.copy(self)
        state_set.sampler = SequenceSampler(
            shape_meta=self.shape_meta,
            replay_buffer=self.replay_buffer,
            rgb_keys=self.rgb_keys,
            humandim_keys=self.humandim_keys,
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        torch_data = {
            ...
            'state': torch.from_numpy(data['future_state'].astype(np.float32))
        }
        return torch_data
