...
from diffusion_policy.policy.state_diffusion_unet_timm_policy import StateDiffusionUnetTimmPolicy
...
class TrainStateDiffusionUnetImageWorkspace(BaseWorkspace):
    ...
    def __init__(self, cfg: OmegaConf, output_dir=None):
        ...
        # configure model
        self.model: StateDiffusionUnetTimmPolicy = hydra.utils.instantiate(cfg.policy)

        self.ema_model: StateDiffusionUnetTimmPolicy = None
        ...
        param_groups = [
            ...
            {'params': self.model.state_embedding.parameters(), 'lr': obs_encorder_lr},
            {'params': self.model.state_estimator.parameters(), 'lr': obs_encorder_lr},
            ...
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                if (self.epoch % cfg.training.val_every) == 0 and accelerator.is_main_process:
                    with torch.no_grad():
                        ...
                        pred = policy.predict_action(batch['obs'], None, eval_real=False)

                        log_action_mse(step_log, 'train', pred['action_pred'], gt_action)
                ...
                if (self.epoch % cfg.training.val_every) == 0 and len(val_dataloader) > 0 and accelerator.is_main_process:
                    with torch.no_grad():
                        gt_action_list, gt_state_list = [], []
                        pred_action_list, pred_state_list, pred_state_logit_list = [], [], []
                        with tqdm.tqdm(val_dataloader, desc=f"Validation epoch {self.epoch}",
                            ...
                            for batch_idx, batch in enumerate(tepoch):
                                ...
                                gt_action_list.append(batch['action'].detach().cpu())
                                gt_state_list.append(batch['state'].detach().cpu())

                                pred_action_list.append(pred['action_pred'].detach().cpu())
                        ...
                        log_state_succ(step_log, 'val', pred_state, gt_state)
                        log_state_loss(step_log, 'val', pred_state_logit, gt_state)
...
def main(cfg):
    workspace = TrainStateDiffusionUnetImageWorkspace(cfg)
    workspace.run()
