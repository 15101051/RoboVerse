...
class TrainDiffusionUnetImageWorkspace(BaseWorkspace):
    ...
    def run(self):
        ...
        使用缓存 = self.cfg.use_cache


        # ###!!!实验性代码
        # if 使用缓存 == 1:
        #     del dataset.replay_buffer
        #     del dataset.sampler.replay_buffer
        #     del val_dataset.replay_buffer
        #     del val_dataset.sampler.replay_buffer
        # ###!!!实验性代码
        ...
        if 使用缓存 == 1:
           normalizer = joblib.load(path_save_data+'.norm')
        else:
        ...
        if 使用缓存 == 1:
            print('训练集缓存字典大小:', dataset.return_dict.get_key_count())
            print('验证集缓存字典大小:', val_dataset.return_dict.get_key_count())
        '''
                # configure validation dataset
        val_dataset = dataset.get_validation_dataset()
        val_dataloader = DataLoader(val_dataset, **cfg.val_dataloader)
            ...
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                with tqdm.tqdm(train_dataloader, desc=f"Training epoch {self.epoch}",
                    ...
                    for batch_idx, batch in enumerate(tepoch):
                        ...
                        # compute loss
                        raw_loss = self.model(batch,使用缓存=1)
                ...
                if (self.epoch % cfg.training.sample_every) == 0 and accelerator.is_main_process:
                    with torch.no_grad():
                        ...
                        if(使用缓存==1):
                            ...
                            pred_action = policy.predict_action(global_cond, None,使用缓存=1)['action_pred']
                        else:
                                ...
                                with torch.no_grad():
                                    ...
                                    with tqdm.tqdm(val_dataloader, desc=f"Validation epoch {self.epoch}",
                                        ...
                                        for batch_idx, batch in enumerate(tepoch):
                                            ...
                                            if (使用缓存 == 1):
                                                global_cond = batch['global_cond']
                                                pred_action = policy.predict_action(global_cond, None, 使用缓存=1)[
                                                    'action_pred']
                                            else:
                                                pred_action = policy.predict_action(batch['obs'], None)['action_pred']
