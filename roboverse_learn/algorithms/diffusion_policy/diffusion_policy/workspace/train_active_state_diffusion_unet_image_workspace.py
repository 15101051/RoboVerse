...
from active_learning.auto_segment import auto_tagging
from active_learning.core_set_policy import CoreSetPolicy
from accelerate import Accelerator
import pandas as pd
OmegaConf.register_new_resolver("eval", eval, replace=True)

class TrainActiveStateDiffusionUnetImageWorkspace(BaseWorkspace):
    ...
    def run(self):
        ...
        policy.eval()
        video_manager = VideoManager(dataset, policy, cache_dir=cfg.task.dataset_path)
        active_policy = CoreSetPolicy(video_manager)
        target_video_idx = active_policy.run()
...
def main(cfg):
    workspace = TrainActiveStateDiffusionUnetImageWorkspace(cfg)
    workspace.run()
