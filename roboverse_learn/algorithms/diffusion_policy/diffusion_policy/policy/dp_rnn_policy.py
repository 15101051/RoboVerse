...
class DPRNNPolicy(BaseImagePolicy):
    def __init__(self,
            shape_meta: dict,
        ...
        # get feature dim
        self.rnn = BCRNN(shape_meta, obs_feature_dim, 1, device)
    ...
    def conditional_sample(self,
        ...
        for t in scheduler.timesteps:
            ...
            model_output = self.diffusion_head(trajectory, t, local_cond=local_cond, global_cond=global_cond)
    ...
    def predict_action(self, obs_dict: Dict[str, torch.Tensor],
        ...
        global_cond = self.rnn.predict_action(nobs)['actions'].unsqueeze(1)
        # empty data for action
        cond_data = torch.zeros(size=(batch_size, self.action_horizon, self.action_dim), device=self.device, dtype=self.dtype)
    ...
    def compute_loss(self, batch):
        ...
        global_cond = self.rnn.predict_action(robomimic_batch)['actions'].squeeze(1)
        ...
        pred = self.diffusion_head(noisy_trajectory, timesteps, local_cond=None, global_cond=global_cond)
    ...
    def get_parameters(self):
        return [
            {'params': self.rnn.nets.parameters()},
            {'params': self.diffusion_head.parameters()}
        ]
