...
def get_val_mask(n_episodes, val_ratio, seed=0, return_idx=False):
    ...
    print(f"val_idxs: {val_idxs}") # 71, 41, 61, 8, 95
    val_mask[val_idxs] = True
    if return_idx:
        return val_mask, val_idxs
    else:
        return val_mask

def get_val_state_mask(n_episodes, val_ratio, state_ratio, seed=42):
    val_mask = np.zeros(n_episodes, dtype=bool)
    state_mask = np.zeros(n_episodes, dtype=bool)
    if val_ratio + state_ratio <= 0:
        return val_mask

    # have at least 1 episode for validation, and at least 1 episode for train
    n_total = min(max(2, round(n_episodes * (val_ratio + state_ratio))), n_episodes-1)
    rng = np.random.default_rng(seed=seed)
    total_idxs = rng.choice(n_episodes, size=n_total, replace=False)
    n_val = max(1, round(n_episodes * val_ratio))
    val_idxs = total_idxs[:n_val]
    state_idxs = total_idxs[n_val:]
    # state_idxs = [72,73,74,71,75,76,94]
    # val_idxs = [i for i in range(n_episodes) if i % 2 == 0 and i not in state_idxs]
    print(f"seed={seed} n_episodes: {n_episodes}\nval_idxs: {val_idxs} state_idx: {state_idxs}") # 71, 41, 61, 8, 95
    val_mask[val_idxs] = True
    state_mask[state_idxs] = True
    return val_mask, state_mask
...
class SequenceSampler:
    def __init__(self,
        ...
        # load gripper_width
        gripper_width = np.asarray(replay_buffer['robot0_gripper_width'])[:, 0]
        ...
        if 'state' in replay_buffer and 'state' in humandim_keys:
            self.states = np.array(self.replay_buffer['state'])
            self.huamndim_dict['state'] = self.states
    ...
    def sample_sequence(self, idx):
        ...
        future_states = np.array(action_horizon * [0])
        if 'state' in self.replay_buffer:
            future_states = self.states[current_idx: slice_end: action_down_sample_steps]
        # solve padding
        if not self.action_padding:
            assert output.shape[0] == action_horizon
            assert future_states.shape[0] == action_horizon
            ...
            padding = np.repeat(future_states[-1:], action_horizon - future_states.shape[0], axis=0)
            future_states = np.concatenate([future_states, padding], axis=0)
        result['action'] = output
        if 'state' in self.replay_buffer and 'state' in self.humandim_keys:
            result['future_state'] = future_states[:self.state_horizon]
