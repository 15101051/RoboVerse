...
class UvcCamera(mp.Process):
    ...
    def __init__(
            ...
            capture_fps=60,
            put_fps=None,
            put_downsample=True, # eval_real.py 默认的环境下，bimanual_umi_env.py里设置为False
            get_max_k=30,
            receive_latency=0.0,
            cap_buffer_size=1,
            num_threads=2,
    ...
    def run(self):
        ...
        try:
            ...
            while not self.stop_event.is_set():
                ...
                data['color'] = frame  # shape = (1080, 1920, 3)

                # 保存原始图像
                frame_filename = "original_image.jpg"  # 或者是你想要的文件名
                # cv2.imwrite(frame_filename, frame)  # 转换为BGR格式后保存
                put_data = data
                if self.transform is not None:
                    put_data = self.transform(dict(data))  # (224, 224, 3)
                    transformed_image = put_data['color']  # 经过转换后的图像
                    
                    # 如果图像是浮点型并且范围在 [0, 1] 之间，转换回 [0, 255]
                    if transformed_image.dtype == np.float32 or transformed_image.dtype == np.float64:
                        transformed_image = np.clip(transformed_image * 255.0, 0, 255).astype(np.uint8)
                    
                    transformed_filename = "transformed_image.jpg"  # 或者是你想要的文件名
                    # cv2.imwrite(transformed_filename, transformed_image)  # 转换为BGR后保存
                if self.put_downsample:   # false             
                    # put frequency regulation
                    ...
                    put_data['timestamp'] = t_cal
                    self.ring_buffer.put(put_data, wait=False)
                    # print(f"[uvc_camera] ring_buffer put data , cap timestamp = {t_cal}")
