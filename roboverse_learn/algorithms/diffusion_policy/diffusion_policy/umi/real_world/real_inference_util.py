...
def get_real_umi_obs_dict(
    ...
    for key, attr in obs_shape_meta.items():
        ...
        if type == 'rgb':
            ...
            assert ci == co
            out_imgs = this_imgs_in
            # print(f"shape {shape} \n in_imgs {this_imgs_in.shape} {this_imgs_in.dtype}")
            if (ho != hi) or (wo != wi) or (this_imgs_in.dtype == np.uint8):
                tf = get_image_transform(
                    input_res=(wi,hi), 
                    output_res=(wo,ho), 
                    bgr_to_rgb=False)
                out_imgs = np.stack([tf(x) for x in this_imgs_in])
                # print(f"out_imgs {out_imgs.shape} {out_imgs.dtype}")
                if this_imgs_in.dtype == np.uint8:
                    out_imgs = out_imgs.astype(np.float32) / 255
            # THWC to TCHW
            obs_dict_np[key] = np.moveaxis(out_imgs,-1,1)
...
def get_real_umi_action(
        action: np.ndarray, # 输入的机器人动作数组
        env_obs: Dict[str, np.ndarray],  # 环境观测字典，包含末端执行器位置和旋转轴角
        action_pose_repr: str='abs' # 动作姿态表示形式，默认为绝对表示
    ):

    n_robots = int(action.shape[-1] // 10) # 计算机器人数量，每个机器人动作占据10个维度
    env_action = list() # 初始化环境动作列表
    for robot_idx in range(n_robots):
        # convert pose to mat，将末端执行器的位置和旋转角度转换为4*4位姿矩阵
        pose_mat = pose_to_mat(np.concatenate([
            env_obs[f'robot{robot_idx}_eef_pos'][-1], # 末端执行器的当前位置
            env_obs[f'robot{robot_idx}_eef_rot_axis_angle'][-1] # 末端执行器的当前旋转轴角
        ], axis=-1))

        start = robot_idx * 10 # 当前机器人的动作起始索引
        action_pose10d = action[..., start:start+9] # 获取当前机器人的9维姿态动作
        action_grip = action[..., start+9:start+10] # 获取当前机器人的1维抓取动作
        action_pose_mat = pose10d_to_mat(action_pose10d) # 将9维姿态动作转换为4*4位姿矩阵
        # print(f"pose_mat {pose_mat.shape} {pose_mat}")
        # print(f"action_pose_mat {action_pose_mat.shape} {action_pose_mat}")
        # solve relative action，计算相对动作
        action_mat = convert_pose_mat_rep(
            action_pose_mat, 
            base_pose_mat=pose_mat, # 基础位姿矩阵，即当前末端执行器的位姿
            pose_rep=action_pose_repr, # 使用的位姿表示形式（绝对或者相对）
            backward=True) # 表示转换方向，从末端执行器当前姿态转换回去
        # print(f"action_mat {action_mat.shape} {action_mat}")
        # convert action to pose
        action_pose = mat_to_pose(action_mat)
        env_action.append(action_pose) # 将计算出的姿态动作添加到环境动作列表中
        env_action.append(action_grip) # 将抓取动作添加到环境动作列表中
