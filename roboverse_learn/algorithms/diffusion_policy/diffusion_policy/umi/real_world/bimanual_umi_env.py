...
from umi.real_world.gripper_controller import WSGController
...
from umi.common.human_operation import HumanOperation, HumanSignal
...
class BimanualUmiEnv:
    def __init__(self, 
            ...
            gripper_down_sample_steps=10,       # omnipicker爪子响应频率慢 TODO：测试改回 1 可否正常运行
            # all in steps (relative to frequency)
        ...
        self.human_operations: List[HumanOperation] = []
    ...
    def get_obs(self) -> dict:
        ...
        # print('==>k  ', k, self.camera_obs_horizon, self.camera_down_sample_steps, self.frequency) # ==>k   8 1 1 10.0
        ...
        for camera_idx, value in self.last_camera_data.items():
            ...
            # remap key
            camera_obs[f'camera{camera_idx}_rgb'] = value['color'][this_idxs] ###这里可以看一眼推理的瞬间所用的obs里面的图片，是不是合理的
        ...
        return obs_data # 这个方法返回了一个字典，包含了所有的obs数据，包括camera的rgb数据，机器人的末端位置和姿态，以及夹爪的宽度，还有时间戳。
        ...
        if self.obs_accumulator is not None:
            ...
            if n_steps > 0:
                ...
                episode = {
                    ...
                    'human_operation': HumanOperation.interpolate_human_operation(timestamps=timestamps, human_operation_list=self.human_operations),
                }
    ...
    def mark_video(self, is_success, mark_time):
        print(f'Mark video at {mark_time}')
        self.human_operations.append(HumanSignal(mark_time, 'Succeed' if is_success else 'Fail'))
